...
// OrbitCLITheme implements the Theme interface with Orbit CLI brand colors.
// It provides both dark and light variants.
type OrbitCLITheme struct {
	BaseTheme
}

// NewOrbitCLITheme creates a new instance of the Orbit CLI theme.
func NewOrbitCLITheme() *OrbitCLITheme {
	// Orbit CLI color palette
	// Dark mode colors - deep blue-purple professional theme
	darkBackground := "#100F0F"
	darkCurrentLine := "#1C1B1A"
	darkSelection := "#282726"
	darkForeground := "#e3e7f5"
	darkComment := "#6b7394"
	darkPrimary := "#7aa2f7"   // Bright blue
	darkSecondary := "#bb9af7" // Soft purple
	darkAccent := "#6b9ef5"    // Blue accent (no cyan)
	darkRed := "#f7768e"
	darkOrange := "#ff9e64"
	darkGreen := "#205EA6"
	darkCyan := "#7aa2f7"
	...
	theme.BackgroundDarkerColor = lipgloss.AdaptiveColor{
		Dark:  "#0d1017", // Slightly darker than background
...
func init() {
	// Register the Orbit CLI theme with the theme manager
	RegisterTheme("orbit-cli", NewOrbitCLITheme())
}
