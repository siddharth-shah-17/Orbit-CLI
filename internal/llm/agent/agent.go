...
import (
	...
	"github.com/svector-corporation/orbit-cli/internal/llm/prompt"
	"github.com/svector-corporation/orbit-cli/internal/llm/provider"
	...
	"github.com/svector-corporation/orbit-cli/internal/permission"
	"github.com/svector-corporation/orbit-cli/internal/pubsub"
	"github.com/svector-corporation/orbit-cli/internal/session"
)
...
func createAgentProvider(agentName config.AgentName) (provider.Provider, error) {
	...
	// Log provider state to aid debugging when UI triggers a send
	logging.Info("creating agent provider", "agent", agentName, "model", model.ID, "provider", model.Provider, "provider_disabled", providerCfg.Disabled, "has_api_key", providerCfg.APIKey != "")

	if providerCfg.Disabled {
		return nil, fmt.Errorf("provider %s is not enabled (check config providers.%s)", model.Provider, model.Provider)
	}
	// Allow SVECTOR to be used without an API key when a BaseURL is configured (local server)
	if providerCfg.APIKey == "" {
		if !(model.Provider == models.ProviderSVECTOR && providerCfg.BaseURL != "") {
			return nil, fmt.Errorf("provider %s is not enabled (check config providers.%s)", model.Provider, model.Provider)
		}
	}
	...
	// If SVECTOR provider has extra options configured, pass them through
	if model.Provider == models.ProviderSVECTOR {
		var svOpts []provider.SVECTOROption
		if providerCfg.BaseURL != "" {
			svOpts = append(svOpts, provider.WithSVECTORBaseURL(providerCfg.BaseURL))
		}
		if len(providerCfg.ExtraHeaders) > 0 {
			svOpts = append(svOpts, provider.WithSVECTORExtraHeaders(providerCfg.ExtraHeaders))
		}
		if len(svOpts) > 0 {
			opts = append(opts, provider.WithSVECTOROptions(svOpts...))
		}
	}
